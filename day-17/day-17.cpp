...
*/ 
int find_shortest_path(const Grid<int>& grid, int min_straight_steps = 0, int max_straight_steps = 3, std::function<int(const Vec2<int>&, const Vec2<int>&)> heuristic = no_heuristic)
{
    ...
    const State start_r = {.pos = {0, 0}, .dir = dir_right, .straight_cnt = 0}; 
    const State start_d = {.pos = {0, 0}, .dir = dir_down, .straight_cnt = 0};
    ...
    while (!queue.empty()) {
        ...
        for (const State& adj: find_adjacent(grid, current, min_straight_steps, max_straight_steps)) {
            const int cost_adj_min = cost.contains(adj) ? cost.at(adj) : infinity;
            if (const int cost_new = cost_current + grid.at(adj.pos); cost_new <= cost_adj_min) {
...
int part_one(const std::vector<std::string>& lines)
{
    ...
    return find_shortest_path(grid, 0, 3, no_heuristic);
}
...
int part_two(const std::vector<std::string>& lines)
{
    Grid<int> grid;
    parse_grid(lines, grid);
    return find_shortest_path(grid, 4, 10, no_heuristic);
}
